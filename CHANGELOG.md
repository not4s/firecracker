...
- Fixed `virtio-mem` leaving its block accounting inconsistent with the KVM
  memory slots if a plug or unplug request failed part way through. Each slot's
  state is now committed only after its KVM update succeeds, so a partial
