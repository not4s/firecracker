...
impl VirtioMem {
    ...
    /// Returns the block range within a slot (given its first block and block count) that the
    /// request covering `req_blocks` touches, and whether the slot must stay plugged into KVM once
    ...
    fn slot_update(
        ...
        // Stays plugged if any block is plugged once applied: touched blocks take `plug`, the rest
        ...
        (touched, keep_plugged)
    }

    /// Applies the request to a single KVM slot: updates KVM first, then, only if that succeeds,
    /// commits the block state for `touched` and (when unplugging) discards it. Committing after
    /// the KVM update keeps the block state consistent with KVM on a partial failure, and a block
    /// is never discarded before it is committed as unplugged.
    ///
    ...
    fn update_range(&mut self, range: &RequestedRange, plug: bool) -> Result<(), VirtioMemError> {
        ...
        for (slot_num, first_block, block_count) in slots {
            let (touched, keep_plugged) =
                self.slot_update(first_block, block_count, &req_blocks, plug);
            discard_failed |= self.apply_to_slot(slot_num, touched, keep_plugged, plug)?;
        }
        // Count at most one discard failure per request, matching the previous behaviour.
        if discard_failed {
