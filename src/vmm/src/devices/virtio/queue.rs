...
impl DescriptorChain {
    ...
    /// Returns the guest address of the descriptor table.
    pub fn desc_table_addr(&self) -> GuestAddress {
        self.desc_table
    }

    /// Returns the total byte size of the descriptor table.
    ...
    /// Gets the next descriptor in this descriptor chain, if there is one.
    /// Falls back to full GPA translation (for callers without a cached region).
...
pub struct Queue {
    ...
    /// Cached translations for the three ring areas (set during initialize).
...
impl PartialEq for Queue {
    fn eq(&self, other: &Self) -> bool {
        self.max_size == other.max_size
            ...
            && self.desc_table_address == other.desc_table_address
            && self.avail_ring_address == other.avail_ring_address
            && self.used_ring_address == other.used_ring_address
...
impl Queue {
    ...
    fn check_alignment(addr: GuestAddress, alignment: u64) -> Result<(), QueueError> {
        if addr.0 & (alignment - 1) != 0 {
            return Err(QueueError::PointerNotAligned(addr.0 as usize, alignment as usize));
        }
        Ok(())
    }
    ...
    pub fn initialize(&mut self, mem: &GuestMemoryMmap) -> Result<(), QueueError> {
        ...
        // Verify alignment per VirtIO spec 1.2 Section 2.6:
        ...
        // Create cached translations for each ring area.
        // This validates that each range fits within a single memory region.
...
mod verification {
    ...
    use super::*;
    use crate::vstate::memory::{Bytes, FileOffset, GuestAddress, GuestMemory, GuestMemoryMmap, MmapRegion};
...
mod tests {
    ...
    fn test_checked_new_descriptor_chain() {
        ...
        // Finally, let's test an ok chain.
        {
            ...
            assert!(c.next_descriptor(m).unwrap().next_descriptor(m).is_none());
        }
    }
    ...
    fn test_queue_processing() {
        ...
        let d = q.pop().unwrap().unwrap().next_descriptor(m).unwrap();
        assert!(!d.has_next());
        assert!(d.next_descriptor(m).is_none());
    ...
    fn test_invalid_avail_idx_no_notification() {
        ...
        let d = q.pop().unwrap().unwrap().next_descriptor(m);
