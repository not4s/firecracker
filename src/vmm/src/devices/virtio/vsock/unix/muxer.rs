...
impl VsockMuxer {
    ...
    /// Connect to the host-side Unix socket at `path` with a non-blocking socket.
    ///
    /// Fails with `EAGAIN` instead of waiting when the listener's accept backlog is full.
    fn connect_nonblocking(path: &str) -> std::io::Result<UnixStream> {
        // SAFETY: socket() is called with constant arguments and the return value is checked.
        let fd = unsafe { libc::socket(libc::AF_UNIX, libc::SOCK_STREAM | libc::SOCK_CLOEXEC, 0) };
        if fd < 0 {
            return Err(std::io::Error::last_os_error());
        }
        // SAFETY: We just checked that the fd is valid, and nothing else owns it.
        let stream = unsafe { UnixStream::from_raw_fd(fd) };
        stream.set_nonblocking(true)?;

        // SAFETY: sockaddr_un is a plain C struct for which all-zero bytes is a valid value.
        ...
        // SAFETY: connect() is called with a valid fd and a pointer to an initialised
        // sockaddr_un together with its length; the return value is checked.
        let ret = unsafe {
            libc::connect(
                stream.as_raw_fd(),
...
mod tests {
    ...
    fn test_peer_connection_backlog_full() {
        ...
        let listener = ctx.create_local_listener(LOCAL_PORT);
        // Shrink the accept backlog: std sizes it to `somaxconn`, and filling that would
        // exceed the fd limit the tests run under.
        // SAFETY: listen() on a valid, already listening fd; the return value is checked.
        assert_eq!(unsafe { libc::listen(listener.sock.as_raw_fd(), 0) }, 0);

        // Fill the listener's accept backlog so that a further connect would block.
