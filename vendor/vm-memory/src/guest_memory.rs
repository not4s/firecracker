...
impl<T: GuestMemory + ?Sized> Bytes<GuestAddress> for T {
    ...
    #[inline]
    fn read_obj<U: crate::bytes::ByteValued>(&self, addr: GuestAddress) -> Result<U> {
        if let Some(mem) = self.physical_memory() {
            if let Some((region, region_addr)) = mem.to_region_addr(addr) {
                if region
        ...
        // Fallback: object spans region boundary or IOMMU translation in place
    ...
    /// fits within a single region (the common case).
    #[inline]
    fn write_obj<U: crate::bytes::ByteValued>(&self, val: U, addr: GuestAddress) -> Result<()> {
        if let Some(mem) = self.physical_memory() {
            if let Some((region, region_addr)) = mem.to_region_addr(addr) {
                if region
                    .checked_offset(region_addr, core::mem::size_of::<U>() - 1)
